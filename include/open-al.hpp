...
/**
 * @brief Wrapper for calls to OpenAL functions.
 *
 * Calls that go through this wrapper will be checked for errors. Use
 * this wrapper always for makeing calls to OpenAL.
 */
#define AL_CALL(function, ...)                                                 \
  al_call_impl(__FILE__, __LINE__, function, __VA_ARGS__)

/**
 * @brief Wrapper for calls to OpenAL context functions.
 *
 * Calls that go through this wrapper will be checked for errors. Use
 * this wrapper always for makeing calls to OpenAL.
 */
...
auto al_call_impl(const char *             filename,
                  const std::uint_fast32_t line,
                  alFunction               function,
                  Params... params) ->
...
auto alc_call_impl(const char *             filename,
                   const std::uint_fast32_t line,
